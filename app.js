// ─── GastroWawi PWA – Großküchen-Warenwirtschaft ─────────────────────────────
// Helles, touch-optimiertes Design für Tablet-Einsatz in der Küche
...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { font-family: 'Nunito', sans-serif; color: ${C.text}; font-size: 15px; }
  ...
  /* ── TOP BAR ── */
  .topbar {
    background: ${C.blue}; color: white; padding: 0 20px;
    height: 60px; display: flex; align-items: center; gap: 14px;
    flex-shrink: 0; box-shadow: 0 2px 12px #2563EB33;
    position: relative; z-index: 10;
  }
  .topbar-menu { background: none; border: none; color: white; font-size: 22px; cursor: pointer;
    width: 44px; height: 44px; border-radius: 10px; display: flex; align-items: center; justify-content: center;
    transition: background 0.15s; flex-shrink: 0; }
  .topbar-menu:hover { background: #ffffff22; }
  .topbar-logo { font-size: 19px; font-weight: 900; letter-spacing: -0.3px; flex: 1; }
  .topbar-logo span { font-size: 14px; font-weight: 600; opacity: 0.75; margin-left: 6px; }
  ...
  .topbar-badge.red { background: #DC262622; color: #FCA5A5; border: 1px solid #DC262633; }
  .topbar-badge.yellow { background: #D9770622; color: #FDE68A; border: 1px solid #D9770633; }
  .topbar-time { font-size: 13px; font-weight: 700; opacity: 0.8; white-space: nowrap; }

  /* ── BOTTOM NAV (mobile/tablet) ── */
  .bottom-nav {
    position: fixed; bottom: 0; left: 0; right: 0; z-index: 20;
    background: white; border-top: 1.5px solid ${C.border};
  ...
  .bn-item { flex: 1; display: flex; flex-direction: column; align-items: center;
    ...
    color: ${C.textLight}; font-size: 10px; font-weight: 700; gap: 3px; border: none;
  ...
  /* ── CONTENT ── */
  .content { flex: 1; overflow-y: auto; padding: 16px; padding-bottom: calc(72px + env(safe-area-inset-bottom, 0px)); }
  ...
  .kpi-val { font-family: 'JetBrains Mono', monospace; font-size: 26px; font-weight: 700;
    line-height: 1; color: ${C.text}; }
  .kpi-lbl { font-size: 12px; font-weight: 700; color: ${C.textMid}; margin-top: 5px;
    text-transform: uppercase; letter-spacing: 0.5px; }
  .kpi-sub { font-size: 12px; color: ${C.textLight}; margin-top: 4px; font-weight: 600; }

  /* ── ALERT BARS ── */
  .alert { display: flex; align-items: center; gap: 12px; padding: 13px 16px; border-radius: 14px;
  ...
  .btn { display: inline-flex; align-items: center; justify-content: center; gap: 7px;
    border-radius: 12px; font-size: 14px; font-weight: 800; cursor: pointer; border: none;
    transition: all 0.15s; font-family: 'Nunito', sans-serif; white-space: nowrap;
    min-height: 44px; padding: 0 18px; }
  .btn:active { transform: scale(0.97); }
  .btn-primary { background: ${C.blue}; color: white; box-shadow: 0 2px 8px #2563EB33; }
  ...
  .btn-lg { min-height: 52px; font-size: 16px; padding: 0 24px; border-radius: 14px; }
  .btn-xl { min-height: 64px; font-size: 17px; padding: 0 28px; border-radius: 16px; width: 100%; }
  .btn-sm { min-height: 34px; font-size: 12px; padding: 0 12px; border-radius: 8px; }
  .btn-icon { width: 44px; min-height: 44px; padding: 0; border-radius: 10px; }
  ...
  .action-tile { background: white; border: 1.5px solid ${C.border}; border-radius: 20px;
    ...
    min-height: 100px; }
  .action-tile:hover { border-color: ${C.blue}; box-shadow: 0 4px 16px #2563EB1A; }
  .action-tile:active { transform: scale(0.98); }
  .action-tile.blue { background: ${C.bluePale}; border-color: #BFDBFE; }
  .action-tile.green { background: ${C.greenLight}; border-color: #BBF7D0; }
  .action-tile.orange { background: ${C.orangeLight}; border-color: #FED7AA; }
  .action-tile.purple { background: ${C.purpleLight}; border-color: #C4B5FD; }
  ...
  input:focus, select:focus, textarea:focus { border-color: ${C.blue}; background: white; }
  ...
  /* ── MODAL ── */
  .modal-overlay { position: fixed; inset: 0; background: #0F172A88; z-index: 100;
  ...
  .modal-handle { width: 40px; height: 4px; background: ${C.border}; border-radius: 2px;
    margin: 12px auto 0; }
  .modal-head { display: flex; align-items: center; justify-content: space-between;
    padding: 16px 20px 14px; }
  .modal-title { font-size: 19px; font-weight: 900; }
  .modal-body { padding: 0 20px; display: flex; flex-direction: column; gap: 14px; padding-bottom: 4px; }
  ...
  /* ── QUANTITY PICKER ── */
  .qty-picker { display: flex; align-items: center; gap: 0; border: 2px solid ${C.border}; border-radius: 14px; overflow: hidden; }
  .qty-btn { width: 54px; height: 54px; background: ${C.bg}; border: none; font-size: 22px;
    font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center;
    color: ${C.blue}; transition: background 0.1s; flex-shrink: 0; }
  .qty-btn:active { background: ${C.blueLight}; }
  .qty-val { flex: 1; text-align: center; font-size: 18px; font-weight: 900;
    font-family: 'JetBrains Mono', monospace; background: white; height: 54px;
    display: flex; align-items: center; justify-content: center; border: none;
    border-left: 1px solid ${C.border}; border-right: 1px solid ${C.border}; min-width: 0; }
  .qty-input { width: 100%; text-align: center; font-size: 18px; font-weight: 900;
    font-family: 'JetBrains Mono', monospace; background: white; height: 50px;
    border: none; outline: none; border-left: 1px solid ${C.border}; border-right: 1px solid ${C.border}; min-width: 0; }
  ...
  .stock-val { font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: 700; white-space: nowrap; color: ${C.text}; min-width: 70px; text-align: right; }
  ...
  .bar-lbl { font-size: 9px; font-weight: 800; color: ${C.textLight}; }
  ...
  /* ── REZEPTUR CARD ── */
  .rez-card { background: white; border: 1.5px solid ${C.border}; border-radius: 18px; padding: 16px;
    cursor: pointer; transition: all 0.15s; }
  .rez-card:hover { border-color: ${C.blue}; box-shadow: 0 4px 16px #2563EB15; }
  .rez-card.selected { border-color: ${C.blue}; background: ${C.bgBlue}; box-shadow: 0 4px 20px #2563EB22; }
  ...
  .toast { background: ${C.text}; color: white; padding: 12px 16px; border-radius: 14px;
    font-size: 14px; font-weight: 700; max-width: 320px; box-shadow: 0 4px 20px #00000030;
  ...
  @media (min-width: 640px) {
    ...
    .rez-grid { grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); }
...
const INIT = {
  ...
  rezepturen: [
    { id: 1, name: "Rinderfilet auf Rotweinsoße", kategorie: "Hauptgericht", vkPreis: 28.50, portionen: 1,
      zutaten: [{ artikelId: 1, menge: 0.22, einheit: "kg" }, { artikelId: 4, menge: 0.15, einheit: "Fl" }] },
    { id: 2, name: "Pasta al Salmone", kategorie: "Hauptgericht", vkPreis: 22.00, portionen: 1,
      zutaten: [{ artikelId: 2, menge: 0.12, einheit: "kg" }, { artikelId: 6, menge: 0.08, einheit: "kg" }, { artikelId: 5, menge: 0.02, einheit: "Pkg" }] },
    { id: 3, name: "Tomatensalat", kategorie: "Vorspeise", vkPreis: 9.50, portionen: 1,
      zutaten: [{ artikelId: 3, menge: 0.15, einheit: "Kiste" }] },
    { id: 4, name: "Bratkartoffeln", kategorie: "Beilage", vkPreis: 5.50, portionen: 1,
...
const mhdColor = days => days < 0 ? C.red : days <= 3 ? C.red : days <= 7 ? C.yellow : C.green;
...
// ─── QUANTITY PICKER ──────────────────────────────────────────────────────────
function QtyPicker({ value, onChange, step = 1, min = 0, unit = '' }) {
  const v = Number(value) || 0;
  return (
    <div className="qty-picker">
      <button className="qty-btn" onClick={() => onChange(Math.max(min, +(v - step).toFixed(3)))}>−</button>
      <input className="qty-input" type="number" value={v} min={min} step={step}
        onChange={e => onChange(Number(e.target.value))} />
      <button className="qty-btn" onClick={() => onChange(+(v + step).toFixed(3))}>+</button>
    </div>
  );
}
...
function Dashboard({ data, setData, toast, setPage }) {
  const { artikel, lager, verbrauch, rezepturen } = data;
  ...
  return (
    <div>
      ...
      {/* Quick Actions */}
      <div style={{marginBottom:8}}><span className="sec-title" style={{fontSize:16}}>Schnellzugriff</span></div>
      <div className="action-grid">
        <div className="action-tile blue" onClick={() => setPage('wareneingang')}>
          <div className="tile-icon">🚚</div>
          <div><div className="tile-label">Ware einbuchen</div><div className="tile-sub">Wareneingang erfassen</div></div>
        </div>
        <div className="action-tile green" onClick={() => setPage('rezepturen')}>
          <div className="tile-icon">👨‍🍳</div>
          <div><div className="tile-label">Produktion starten</div><div className="tile-sub">Rezeptur abbuchen</div></div>
        </div>
        <div className="action-tile orange" onClick={() => setPage('inventur')}>
          <div className="tile-icon">🔢</div>
          <div><div className="tile-label">Inventur</div><div className="tile-sub">Bestände zählen</div></div>
        </div>
        <div className="action-tile purple" onClick={() => setPage('lager')}>
          <div className="tile-icon">📦</div>
      ...
      <div className="card">
        <div className="card-head">
          <span className="card-title">📊 Wochenverbrauch (Wareneinsatz €)</span>
        </div>
        <div style={{padding: '16px 16px 12px'}}>
          <div className="bar-chart">
            {mockBars.map((v, i) => (
              <div key={i} className="bar-col">
                <div className="bar-fill" style={{ height: `${(v/maxB)*100}%`, background: i === 4 ? C.blue : C.blueLight, border: `1.5px solid ${i===4?C.blue:C.border}` }} />
              </div>
            ))}
          </div>
          <div style={{display:'flex',gap:6,marginTop:4}}>
            {wochentage.map(d => <div key={d} className="bar-lbl" style={{flex:1,textAlign:'center'}}>{d}</div>)}
          </div>
          <div style={{display:'flex',gap:24,marginTop:12,paddingTop:10,borderTop:`1px solid ${C.border}`}}>
            <div><div style={{fontSize:11,color:C.textLight,fontWeight:700,textTransform:'uppercase'}}>Ø / Tag</div>
              <div className="mono" style={{fontSize:16,fontWeight:700,color:C.blue}}>{fmtE(mockBars.reduce((a,b)=>a+b,0)/7)}</div></div>
            <div><div style={{fontSize:11,color:C.textLight,fontWeight:700,textTransform:'uppercase'}}>Gesamt Woche</div>
              <div className="mono" style={{fontSize:16,fontWeight:700}}>{fmtE(mockBars.reduce((a,b)=>a+b,0))}</div></div>
          </div>
        </div>
      </div>

      {/* MHD-Übersicht */}
      {lager.filter(l => daysDiff(l.mhd) <= 7 && daysDiff(l.mhd) >= 0).length > 0 && (
        <div className="card">
          ...
          <table className="tbl">
            ...
            <tbody>
              {lager.filter(l => daysDiff(l.mhd) <= 7 && daysDiff(l.mhd) >= 0)
                .sort((a,b) => new Date(a.mhd)-new Date(b.mhd)).map(l => {
...
function Lager({ data, setData, toast }) {
  ...
  return (
    <div>
      ...
      <div className="card">
        <table className="tbl">
          <thead><tr><th>Artikel</th><th>Kategorie</th><th>Bestand / Mindest</th><th>MHD (nächste)</th><th>Lagerwert</th><th>Aktion</th></tr></thead>
          <tbody>
            {rows.length === 0 && <tr><td colSpan={6}><div className="empty"><div className="empty-icon">📦</div><div className="empty-title">Nichts gefunden</div></div></td></tr>}
            {rows.map(a => {
              const np = a.positionen[0];
              const days = np ? daysDiff(np.mhd) : null;
              const pct = a.mindestbestand > 0 ? Math.min((a.bestand / (a.mindestbestand * 3)) * 100, 100) : 100;
              const wert = a.bestand * (np?.ek || a.ek);
              return (
                <tr key={a.id}>
                  <td><div style={{fontWeight:800}}>{a.name}</div>
                  ...
                  <td>{np ? <Badge type={mhdBadge(days)}>{np.mhd} ({mhdLabel(days)})</Badge> : <span style={{color:C.textLight}}>—</span>}</td>
                  <td className="mono" style={{color:C.blue,fontWeight:800}}>{fmtE(wert)}</td>
                  <td>
                    {np && <button className="btn btn-ghost btn-sm" onClick={() => { setAbgangModal({lagerPos:np,art:a}); setAbgangMenge(1); }}>
                      Entnahme
                    </button>}
                  </td>
                </tr>
      ...
      {abgangModal && (
        <Modal title={`Entnahme: ${abgangModal.art.name}`} onClose={() => setAbgangModal(null)}
          footer={<>
            ...
            <button className="btn btn-ghost" onClick={() => setAbgangModal(null)} style={{width:'100%'}}>Abbrechen</button>
          </>}>
          <div className="alert info">
            <span className="alert-icon">📦</span>
            <div className="alert-text">Charge: {abgangModal.lagerPos.charge} · MHD: {abgangModal.lagerPos.mhd}<br/>
          ...
          <div className="form-group">
            ...
            <QtyPicker value={abgangMenge} onChange={setAbgangMenge} step={0.1} min={0.1} unit={abgangModal.art.einheit} />
          </div>
        </Modal>
      )}
    </div>
...
function Wareneingang({ data, setData, toast }) {
  ...
  const [step, setStep] = useState(0); // 0=Liste, 1=Kopf, 2=Positionen
  ...
  function addPos() {
    ...
    setForm(f => ({ ...f, positionen: [...f.positionen, { ...pos, id: Date.now(), artName: art?.name, artEinheit: art?.einheit }] }));
  ...
  function buchen() {
    ...
    const we = {
      id: Date.now(), lieferantId: Number(form.lieferantId), datum: form.datum, status: 'Gebucht',
      belegnr: form.belegnr || `WE-${Date.now().toString().slice(-5)}`,
      positionen: form.positionen.map(p => ({ artikelId: Number(p.artikelId), menge: Number(p.menge), ek: Number(p.ek), mhd: p.mhd })),
    };
    const newLager = form.positionen.map(p => {
      const art = getA(artikel, Number(p.artikelId));
      return { id: Date.now()+Math.random(), artikelId: Number(p.artikelId), menge: Number(p.menge),
        ek: Number(p.ek)||art?.ek||0, mhd: p.mhd||'2026-12-31', lagerort: p.lagerort,
    ...
    toast(`Wareneingang ${we.belegnr} gebucht (${we.positionen.length} Pos.)`, 'success');
    setStep(0);
  ...
  if (step === 1) return (
    <div>
      ...
      <div className="sec-head"><div><div className="sec-title">1. Lieferant & Datum</div><div className="sec-sub">Bitte Lieferant und Lieferdatum auswählen</div></div></div>
      ...
      <div style={{display:'flex',gap:10,marginTop:16}}>
        <button className="btn btn-ghost" style={{flex:1}} onClick={() => setStep(0)}>Abbrechen</button>
  ...
  return (
    <div>
      ...
      <div className="sec-head"><div><div className="sec-title">2. Positionen erfassen</div>
        <div className="sec-sub">{getL(lieferanten, Number(form.lieferantId))?.name} · {form.datum}</div></div></div>
      ...
      <div className="card" style={{padding:16}}>
        <div style={{fontSize:12,fontWeight:800,color:C.textMid,textTransform:'uppercase',letterSpacing:'.5px',marginBottom:12}}>Position hinzufügen</div>
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div className="form-group">
            <label className="form-label">Artikel *</label>
            <select value={pos.artikelId} onChange={e=>{const a=getA(artikel,Number(e.target.value));setPos(p=>({...p,artikelId:e.target.value,ek:a?.ek||''}));}}>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">EK-Preis (€)</label>
              <input type="number" min="0" step="0.01" value={pos.ek} onChange={e=>setPos(p=>({...p,ek:e.target.value}))} /></div>
            <div className="form-group"><label className="form-label">MHD</label>
              <input type="date" value={pos.mhd} onChange={e=>setPos(p=>({...p,mhd:e.target.value}))} /></div>
          </div>
          <div className="form-group"><label className="form-label">Lagerort</label>
            <select value={pos.lagerort} onChange={e=>setPos(p=>({...p,lagerort:e.target.value}))}>
              {lagerorte.map(o=><option key={o}>{o}</option>)}
            </select>
          </div>
          <button className="btn btn-primary" style={{width:'100%'}} onClick={addPos} disabled={!pos.artikelId}>
            + Position hinzufügen
          </button>
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:10,marginTop:16}}>
        ...
        <button className="btn btn-primary" style={{flex:2}} disabled={!form.positionen.length} onClick={buchen}>
          ✅ {form.positionen.length} Positionen buchen
        </button>
      </div>
    </div>
  );
    ...
    <div>
      <div className="rez-grid">
        {rezepturen.map(rez => {
          ...
          return (
            <div key={rez.id} className={`rez-card${selected?.id===rez.id?' selected':''}`} onClick={() => { setSelected(rez); setPortionen(1); }}>
              ...
              <div className="rez-stats">
                <div className="rez-stat"><div className="rez-stat-lbl">Wareneinsatz</div>
                  <div className="rez-stat-val" style={{color:C.red}}>{fmtE(kosten)}</div></div>
                <div className="rez-stat"><div className="rez-stat-lbl">VK-Preis</div>
                  <div className="rez-stat-val" style={{color:C.blue}}>{fmtE(rez.vkPreis)}</div></div>
                <div className="rez-stat"><div className="rez-stat-lbl">Marge</div>
                  <div className="rez-stat-val" style={{color:margeP>60?C.green:margeP>40?C.yellow:C.red}}>{fmt(margeP,1)}%</div></div>
              </div>
            </div>
          );
        })}
      ...
      {selected && (
        <Modal title={`Produktion: ${selected.name}`} onClose={() => setSelected(null)}
          footer={<>
            <button className="btn btn-primary btn-xl" onClick={produzieren}>
              👨‍🍳 {portionen} Portion{portionen!==1?'en':''} produzieren
            </button>
            <button className="btn btn-ghost" style={{width:'100%'}} onClick={() => setSelected(null)}>Abbrechen</button>
          </>}>
          <div className="form-group">
            <label className="form-label">Anzahl Portionen</label>
            <QtyPicker value={portionen} onChange={setPortionen} step={1} min={1} />
          </div>
          <div style={{background:C.bg,borderRadius:14,padding:'14px',border:`1.5px solid ${C.border}`}}>
            <div style={{fontSize:12,fontWeight:800,color:C.textMid,textTransform:'uppercase',letterSpacing:'.5px',marginBottom:10}}>Abbuchung (FIFO)</div>
            {selected.zutaten.map((z,i) => {
              const art = getA(artikel, z.artikelId);
              const benötigt = z.menge * portionen;
              const bestand = getLB(lager, z.artikelId);
              const ok = bestand >= benötigt;
              return (
                <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:i<selected.zutaten.length-1?`1px solid ${C.border}`:'',fontSize:13,fontWeight:700}}>
                  <span style={{color:ok?C.text:C.red}}>{art?.name}</span>
                  <span style={{color:ok?C.textMid:C.red}}>{fmt(benötigt,3)} {z.einheit} {!ok&&`⚠ (nur ${fmt(bestand,2)} verfügbar)`}</span>
                </div>
              );
            })}
            <div style={{display:'flex',justifyContent:'space-between',marginTop:12,paddingTop:10,borderTop:`1.5px solid ${C.border}`}}>
              ...
              <span className="mono" style={{fontWeight:800,color:C.blue,fontSize:16}}>{fmtE(getRezKosten(selected,artikel,lager)*portionen)}</span>
            </div>
          </div>
        </Modal>
      )}
...
function Inventur({ data, setData, toast }) {
  ...
  const rows = artikel.map(a => {
    ...
    return { ...a, soll, ist, diff: ist - soll };
  });
    ...
    setData(d => {
      ...
      rows.forEach(a => {
        ...
        if (a.diff < 0) {
          ...
          newVerbrauch.push({ id: Date.now()+Math.random(), datum: todayStr(), artikelId: a.id,
            menge: Math.abs(a.diff), grund: 'Inventurkorrektur (Schwund)' });
        } else {
          newLager.push({ id: Date.now()+Math.random(), artikelId: a.id, menge: a.diff,
            ek: a.ek, mhd: '2026-12-31', lagerort: 'Unbekannt',
            charge: `INV-${todayStr()}`, eingang: todayStr() });
        }
      });
  ...
  return (
    <div>
      {gebucht && <div className="alert success" style={{marginBottom:14}}>
        <span className="alert-icon">✅</span>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,gap:10,flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          {diffWert !== 0 && <div style={{fontSize:13,fontWeight:800,color:diffWert<0?C.red:C.green}}>
        ...
        {rows.map((a, i) => (
          <div key={a.id} className="inv-row">
            ...
            {counts[a.id] !== undefined && a.diff !== 0 && (
              <div style={{width:80,textAlign:'right',fontWeight:800,fontSize:13,color:a.diff<0?C.red:C.green,flexShrink:0}}>
...
function Bestellungen({ data, setData, toast }) {
  ...
  const vorschlaege = artikel.filter(a => getLB(lager, a.id) < a.mindestbestand).map(a => ({
    ...a, bestand: getLB(lager, a.id), fehlmenge: a.mindestbestand - getLB(lager, a.id)
  }));
  ...
  function bestellenAlle() {
    ...
    const newBest = Object.entries(byLief).map(([lid, pos]) => ({
      id: Date.now()+Math.random(), lieferantId: Number(lid), datum: todayStr(),
      status: 'Offen', belegnr: `BE-${Date.now().toString().slice(-5)}`, positionen: pos
    }));
    ...
    <div>
      {vorschlaege.length > 0 && (
        <>
          ...
          <div className="card">
            ...
            <table className="tbl">
              <thead><tr><th>Artikel</th><th>Lieferant</th><th>Bestand</th><th>Mindest</th><th>Fehlmenge</th><th>Bestellmenge</th><th>EK-Wert</th></tr></thead>
              <tbody>
                {vorschlaege.map(a => {
                  ...
                  return (
                    <tr key={a.id}>
                      ...
                      <td className="mono" style={{color:C.orange}}>{fmt(a.fehlmenge,1)}</td>
      ...
      <div className="card">
        ...
        {bestellungen.length === 0
          ? <div className="empty"><div className="empty-icon">📋</div><div className="empty-title">Keine offenen Bestellungen</div></div>
          : <table className="tbl">
                ...
                {bestellungen.map(b => {
                  ...
                  return (
                    <tr key={b.id}>
                      ...
                      <td>{b.status==='Offen'&&<button className="btn btn-ghost btn-sm" onClick={() =>
                        setData(d => ({...d,bestellungen:d.bestellungen.map(x=>x.id===b.id?{...x,status:'Bestellt'}:x)}))}>
                        Bestätigen
                      </button>}</td>
                    </tr>
                  );
...
function Stammdaten({ data, setData, toast }) {
  ...
  function saveArt() {
    ...
    setData(d => ({...d, artikel: [...d.artikel, {...artForm, id:Date.now(), lieferantId:Number(artForm.lieferantId), mindestbestand:Number(artForm.mindestbestand), ek:Number(artForm.ek), mwst:Number(artForm.mwst)}]}));
  ...
  function saveLief() {
    ...
    setData(d => ({...d, lieferanten: [...d.lieferanten, {...liefForm, id:Date.now(), zahlungsziel:Number(liefForm.zahlungsziel)}]}));
  ...
  return (
    <div>
      ...
      {tab === 'artikel' && (
        <>
          <button className="btn btn-primary" style={{marginBottom:14,width:'100%'}} onClick={()=>setArtModal(true)}>+ Artikel anlegen</button>
          <div className="card">
            <table className="tbl">
      ...
      {tab === 'lieferanten' && (
        <>
          <button className="btn btn-primary" style={{marginBottom:14,width:'100%'}} onClick={()=>setLiefModal(true)}>+ Lieferant anlegen</button>
          <div className="card">
            <table className="tbl">
      ...
      {artModal && (
        <Modal title="Artikel anlegen" onClose={()=>setArtModal(false)} footer={<>
          ...
          <div className="form-group"><label className="form-label">Bezeichnung *</label>
            <input value={artForm.name} onChange={e=>setArtForm(f=>({...f,name:e.target.value}))} placeholder="z.B. Rinderfilet" /></div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">Einheit</label>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">EK-Preis (€)</label>
              <input type="number" min="0" step="0.01" value={artForm.ek} onChange={e=>setArtForm(f=>({...f,ek:e.target.value}))} /></div>
            <div className="form-group"><label className="form-label">MwSt. (%)</label>
              <select value={artForm.mwst} onChange={e=>setArtForm(f=>({...f,mwst:e.target.value}))}>
                <option value={7}>7%</option><option value={19}>19%</option>
              </select></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">Lieferant</label>
              <select value={artForm.lieferantId} onChange={e=>setArtForm(f=>({...f,lieferantId:e.target.value}))}>
                <option value="">— wählen —</option>
                {lieferanten.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}
              </select></div>
            <div className="form-group"><label className="form-label">Mindestbestand</label>
              <input type="number" min="0" step="0.5" value={artForm.mindestbestand} onChange={e=>setArtForm(f=>({...f,mindestbestand:e.target.value}))} /></div>
          </div>
        </Modal>
      )}

      {liefModal && (
        <Modal title="Lieferant anlegen" onClose={()=>setLiefModal(false)} footer={<>
          ...
          <div className="form-group"><label className="form-label">Firmenname *</label>
            <input value={liefForm.name} onChange={e=>setLiefForm(f=>({...f,name:e.target.value}))} /></div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">Kürzel</label>
              <input value={liefForm.kuerzel} onChange={e=>setLiefForm(f=>({...f,kuerzel:e.target.value}))} /></div>
            <div className="form-group"><label className="form-label">Zahlungsziel (Tage)</label>
              <input type="number" value={liefForm.zahlungsziel} onChange={e=>setLiefForm(f=>({...f,zahlungsziel:e.target.value}))} /></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">Kontaktperson</label>
              <input value={liefForm.kontakt} onChange={e=>setLiefForm(f=>({...f,kontakt:e.target.value}))} /></div>
            <div className="form-group"><label className="form-label">Telefon</label>
              <input value={liefForm.telefon} onChange={e=>setLiefForm(f=>({...f,telefon:e.target.value}))} /></div>
          </div>
          <div className="form-group"><label className="form-label">E-Mail</label>
            <input type="email" value={liefForm.email} onChange={e=>setLiefForm(f=>({...f,email:e.target.value}))} /></div>
        </Modal>
      )}
    </div>
  );
...
function App() {
  const [data, setData] = useState(() => {
    try {
      const saved = localStorage.getItem('gastrowawi-data');
  ...
  useEffect(() => {
    try { localStorage.setItem('gastrowawi-data', JSON.stringify(data)); } catch {}
  }, [data]);
      ...
      <div className="app">
        ...
        <div className="topbar">
          <div className="topbar-logo">
            👨‍🍳 GastroWawi <span>Großküche</span>
          </div>
        ...
        {/* PAGE TITLE */}
        <div style={{background:C.blueLight,borderBottom:`1.5px solid ${C.border}`,padding:'10px 16px',flexShrink:0}}>
          <div style={{fontSize:13,fontWeight:800,color:C.blue}}>{PAGES.find(p=>p.id===page)?.icon} {pageTitle}</div>
        </div>
